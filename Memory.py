...
def draw(canvas):
    for i in range(len(deck)):
        if exposed[i] == False:
            canvas.draw_polygon([(i*CANVAS_WIDTH/len(deck), 0), (i*CANVAS_WIDTH/len(deck), CANVAS_HEIGHT), ((i + 1)*CANVAS_WIDTH/len(deck), CANVAS_HEIGHT), ((i + 1)*CANVAS_WIDTH/len(deck), 0), (i*CANVAS_WIDTH/len(deck), 0)], CANVAS_HEIGHT * 0.04, '#2f4f4f', '#9370db')
        else:
            canvas.draw_text(str(deck[i]), (i*CANVAS_WIDTH/len(deck) + 5, CANVAS_HEIGHT * 0.7), CANVAS_HEIGHT * 0.6, '#adff2f', 'monospace')
